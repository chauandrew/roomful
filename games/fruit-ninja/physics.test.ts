...
function fruit(over: Partial<Entity>): Entity {
  return { id: 1, kind: "fruit", x: 0.5, y: 0.5, vx: 0, vy: 0, radius: CONFIG.FRUIT_RADIUS, color: "#f87171", ...over };
}
...
test("spawn interval and launch speed ramp from START to END across the round", () => {
  stubRandom(() => 0.5, () => {
    ...
    const times: number[] = [];
    const speeds: number[] = [];
    while (state.nextSpawnAtMs <= CONFIG.ROUND_DURATION_MS) {
      times.push(state.nextSpawnAtMs);
      const r = spawnDue(state, state.nextSpawnAtMs);
      state = r.state;
    ...
    assert.ok(intervals[0] > 1250 && intervals[0] <= CONFIG.SPAWN_INTERVAL_START_MS);
    assert.ok(intervals[intervals.length - 1] < 500 && intervals[intervals.length - 1] >= CONFIG.SPAWN_INTERVAL_END_MS);

    for (let i = 1; i < speeds.length; i++) assert.ok(speeds[i] > speeds[i - 1]);
    assert.ok(Math.abs(speeds[0] - CONFIG.LAUNCH_SPEED_START) < 0.02);
    assert.ok(Math.abs(speeds[speeds.length - 1] - CONFIG.LAUNCH_SPEED_END) < 0.03);
  });
});
  ...
  const run = (elapsedSteps: number[]) => {
    ...
    stubRandom(lcg(42), () => {
      for (const elapsed of elapsedSteps) {
        const r = spawnDue(state, elapsed);
        state = r.state;
        all.push(...r.spawned);
      }
    });
...
test("a launched fruit arcs: vy flips negative to positive, then falls off and is missed", () => {
  let entities = [fruit({ y: 1 + CONFIG.FRUIT_RADIUS, vy: -CONFIG.LAUNCH_SPEED_START })];
  assert.ok(entities[0].vy < 0);
  let sawPositiveVy = false;
  const missed: Entity[] = [];
  for (let i = 0; i < 100 && entities.length > 0; i++) {
    const r = updateEntities(entities, 50);
    entities = r.entities;
...
test("a bomb falling off the bottom is culled but never counts as missed fruit", () => {
  let entities: Entity[] = [
    { id: 7, kind: "bomb", x: 0.5, y: 1 + CONFIG.BOMB_RADIUS, vx: 0, vy: -CONFIG.LAUNCH_SPEED_START, radius: CONFIG.BOMB_RADIUS, color: "#000" },
  ];
